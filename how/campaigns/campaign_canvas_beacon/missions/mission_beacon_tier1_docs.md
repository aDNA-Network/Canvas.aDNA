...
| **B6** | — | **VISION sub-gate** — replace generic `who/governance/VISION.md` (rec) vs add `VISION_canvas.md`; then author the Canvas-specific vision | operator decision + `MANIFEST` thesis | ⏸ operator sub-gate |
| **O5** | — | Verify (markdown links resolve; no residual `v2.0.0`/stale in touched docs) + SITREP + HOLD at B2→B3 | — | pending |
...
## AAR
_(append the 5-line AAR before status: completed, SO-5)_
