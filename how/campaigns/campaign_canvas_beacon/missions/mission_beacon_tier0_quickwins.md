...
|---|---|-----------|-------------------------|--------|
| **O1** | R0.1 | Fix wrong-repo + dead links | `CONTRIBUTING.md` L21,22,29,49,78,157 (`LatticeProtocol/adna` → `aDNA-Network/Canvas.aDNA`) + dead `README.md` link L159; `who/governance/VISION.md` L149 dead `README.md` link | pending |
| **O2** | R0.2 | Refresh `canvas_std/README.md` | L3 `v2.0.0`→`v2.2.0`; L8 `46 passed / 8 skipped`→`105 passed / 10 skipped`; status line current | pending |
| **O3** | R0.3 | Fix spec title/H1 + stale producer names | `what/specs/spec_adna_canvas_standard.md` L4 (fm `title:`) + L15 (H1) `v2.0.0`→`v2.2.0`; CanvasForge/ComfyForge/SiteForge → Canvas/ComfyUI/Astro | pending |
| **O4** | R0.4 | Document the schema `$id` pin | `src/canvas_std/data/adna_canvas_v2.schema.json` — `$id` deliberately pinned v2.0.0 (structural schema unchanged, CHANGELOG L29); add an inline note rather than bump | pending |
| **O5** | — | Verify + HOLD | `pytest -q` in `what/code/canvas_std/` → expect `105 passed / 10 skipped`; grep links/versions clean; SITREP + HOLD at the B1→B2 gate | pending |
...
## AAR
_(append the 5-line AAR — Worked/Didn't/Finding/Change/Follow-up — before setting status: completed, SO-5)_
