...
| Current Phase | **H0+H1+HV+H2+H5+HF+H4 ✅ · H6 ✅ offline half (partial) · HR 🟡 built (gate 2/3 closed) · H3 ✅ CLOSED — rendered 2026-08-10, eye-gate PASSED 2026-08-13** (the 2026-08-09 'blocked on billing' call was a MISDIAGNOSIS — wrong credential, not missing money). **▶ NEXT GATE: H6 re-open** — unblocked 2026-08-13, the eye-gate was its only dependency; it carries the campaign AAR + close, `CV-COMIC-STYLE-01` calibration (real pixels now preserved at `what/artifacts/h3_first_light/`), real-DPI evidence, and the `canvas_comic` archive `adr_009` already authorizes. **H3 (2026-08-09, session `_h3_first_light`, plan approval = the gate + the SPEND gate + the lane reassignment; `mission_h3_first_light`, `status: partial`):** the operator opened H3, opened the spend gate for the same session, assigned the phase **in full to Mondrian** (dev-lane annex **§3a + Amendment 1** — a *ratified* lane amended in the open, struck-through and dated, never silently taken; no `luke/*` branch had ever existed), and signed **all three pending §7.7 ratifications**. **Built**: geometry-derived aspect — `aspect.py` snaps each panel's true w/h to the **generate backend's own** menu (resolved off a class attribute, so planning never needs a credential); manifest gains `effective_aspect_ratio` + `aspect_snap_error` and **never overwrites the declared value**; every drifting panel is printed by name. **`backends/gemini.py`** — the registry entry that had raised `NotImplementedError` since H2 is live. **BLOCKED**: `429 RESOURCE_EXHAUSTED — prepayment credits are depleted`, **account-wide** (verified on three models incl. a text-only call). The **credential is valid** — the aspect probe returned a well-formed 400, which only happens after auth. A top-up is the operator's; the run is then one command, with spend already verified at **$3.62 of the $5 cap** (`gemini-3-pro-image`, $0.134/image 2K × 27). **F-H3-1, fleet-relevant:** `imagen-4.0-*` is **deprecated with a 2026-08-17 shutdown** — eight days out — and `adna_lab`'s `GeminiImageClient`, the fleet's reference image client, targets exactly that family; adapting it verbatim would have shipped a backend with a fortnight to live. **F-H3-2:** the live aspect menu is **14 entries, not the 5 assumed** — obtained free, because an invalid ratio is rejected at validation and the 400 enumerates the valid set; the splash consequently snaps to **2:3 (residual 0.030)** instead of **9:16 (0.140)**, which is backend-owned menus paying for themselves on first use. **S-1..S-4 built** under the ratification that released them: `RLHF_SIGNAL_TYPE_REJECT` had been declared since the bridge was written and was **never emitted**, so a reject-only pass produced *no learning signal at all*; rejects now derive from `responses[]` with a `response_id` dedup key (never a fabricated `selection_id`) and a distinct trap. **S-4 is a GUARD, not a note** — `REJECT_VOCABULARY_CONFIRMED = False` holds every reject out of the shared store until Argus rules on what `accepted` means, with two tests asserting the default holds. Two tests **inverted, not deleted** (one had been asserting the bug). Suites: comic_render 94→**154/2** · canvas_core 841→**863/3** · boundary guard green · **firewall diff 0**. **Campaign close STILL NOT TAKEN — no page has been rendered.**  ◆ Prior:  **H6 (2026-08-09, session `_211323`, plan approval = the gate; `mission_h6_close`, `status: partial`):** the offline half — everything not needing real pixels or the campaign close. Triggered by three unconsumed Callisto memos, one of which **discharged the Bearly P5 evidence dependency** the RLHF work was parked behind. **`spec_rlhf_seam.md`** (`proposed`): Canvas owns the capture substrate / III owns the signal schema; store heterogeneity ruled normative; ISS-vs-III resolved as a **scope conflation** (two objects, one word — no ownership moved); **open decision #4 RULED** — reject → III as `rlhf_signal_type: reject` derived from `responses[]` (the real gap: that constant has existed since the bridge was written and is never emitted, so a reject-only pass produced NO signal at all); S-1..S-4 **deliberately unbuilt** while `proposed`. **`review_dispatch_contract v0` BOUND** — six clauses; **D5** refusal-atomicity adopted from Callisto's `F-S030-1` and credited by name; **D6** venue boundary from ADR-007 mode L; **dispatcher deliberately NOT built** (the venue has not run a batch). **`--profile knowledge-canvas|comic|all`** (H4 finding #4): all 24 findings came from three knowledge-canvas *aesthetic* traps, none a defect — comic **24→0** source, **21→3** rendered, and all 3 survivors are `CV-IMAGE-ASPECT-RATIO-01`, the trap that caught the real H2 drift. **Print E2E**: `export_spread` was written, correct and **UNREACHABLE** (every spread would have squashed 2:1, silently) · CMYK's `except: pass` soft-convert made output **machine-dependent** and this node's ColorSync profiles are why it stayed invisible — both fixed, DPI policy (300 target / 200 floor / warn-never-block / spreads measured against the COMBINED target) written down. **`comic_authoring_contract.md`** (T3′). **`adr_009`**: `canvas_comic` = reader-only freeze now, archive after H3. Suites: canvas_core 824→**841/3** · comic_render 92→**94/1** (the H6-deferred test **inverted**, not deleted) · producers **259** · `canvas_std` **115/10** · cert **11/11** · **firewall diff 0**; E2E `sync_hash c56c73c08428f621` byte-identical. Spread split **verified by looking** — it passed every assertion on two flat green rectangles first (solid colour splits identically however you cut it); re-run with a structured source proved it. **Campaign close NOT taken — H3 has never run.** ◆ Prior: **H4 (2026-08-06, session `_201701`, plan approval = the gate):** `refine:comfy` is a real backend — img2img on the existing `ComfyForgeTier1Adapter` (`refine_image` · `/upload/image` multipart · separate-negative graph · optional LoRA slot + upscale · deterministic seed · named-template patching that **degrades to the built-in graph** when `comic_panel_refine` is absent upstream) + thin binding `comic_render/backends/comfy.py` + chain syntax `<stage>:<backend>[@denoise][/workflow]` + `RefineClient.lora` (dispatch lifts H5's pair-gated `characters[]` entry — closes roadmap R3's "LoRA re-enters via the refine chain"). **Offline E2E chain proof**: 9 panels → 18 fake variants → 18 mocked-comfy refines → 9 selections → rendered canvas revalidates (`c56c73c08428f621`) → 4 composited pages. comic_render 73→**92/1skip** · canvas_core 800→**819/3** · producers **259** · `canvas_std` **115/10** · cert **11/11** · **firewall diff 0**. Contract fixtures `comic_render/tests/fixtures/comfy/`; live smoke behind `-m network`. Wrapper follow-up **#1 closed**, **#2 recommendation staged**. **H4 remainder: the gemini→comfy chain proven LIVE — joins H3.** ◆ Prior (2026-08-04 session `_115855`; plan approval = the H5/HR/HF gates, HV/H2 precedent). **H5**: `comic_generator/compose_input.py` — VisualDNA bundles → enriched ComicInput → per-panel `qualities.characters` → the H2-reserved manifest lift (zero `comic_render` src changes); **LoRA-less compose exercised + tested** (the named exit-criterion test; Stanley-like + Bearly-like fixtures + live smokes); pair-gate trigger⇔lora; comic 100→**123** · sweep **259** · comic_render **73**; Callisto close-notify **staged**. **HR**: `spec_canvas_review_surface.md` (draft) + REAL pilot `what/artifacts/review_surface_pilot/` (6 variants; `adna_native` [OK]; **visual-check 0 findings** after the rail corrected the first-draft geometry) + `review_collect.py` three-sink collector (4-layer idempotency; `{kind: ai}` attribution); canvas_core **800/3**; **gate 2/3 closed** (spec **RATIFIED** [GO wave 2026-08-04, §7.7] · render **agent-confirmed** [session `_174045`: two live catches — restricted-mode-never-rendered + multiSelect block-only → builder fixed, pilot rebuilt; all 8 controls live incl. inside canvas embeds; buttons-vs-toggles resolved, both idioms render]; **remaining: operator review pass**). **HF**: census artifact (21 wrappers; drift = identity/paths, `source_vault` clean) + **NEW `how/federation/federation_index.md`** (Vulcan #3 satisfied) + **5 staged memos** (per-send GOs). **H3 pre-rulings** (recorded in the H3 row + roadmap §4 #1): Gemini pro-image class · 3 variants/panel · $5 cap · `GEMINI_API_KEY` via Home broker · geometry-derived aspect. Firewall diff 0. Origin: parity restored at the GO wave (`4984ecf`; Luke's lane UNBLOCKED); local render-confirm commits ahead pending the next operator-gated batch push. HOLD (SO-1). |
...
| `missions/artifacts/halftone_dev_lanes.md` | Second-developer annex (Luke, Berthier S105): branch/PR flow · review law · lane split — **ratified 2026-08-03** (the per-send GO on the Berthier reply memo was the ratification, per the annex's own terms; this row said "draft pending ratification" until H6 corrected it — the artifact had carried `status: ratified` since the day it was signed) |
| `missions/mission_h6_close.md` | H6 mission (authoring contract · print E2E · RLHF seam · dispatch contract · visual-check profile · `canvas_comic` disposition) — **partial**, campaign close waits on H3 |
