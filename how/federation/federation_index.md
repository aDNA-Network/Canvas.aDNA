...
| `iss/` | Astro.aDNA (ISS subsystem) | **commit `1c51382`** (2026-06-30) | `commit_pin` | **NEW 2026-09-16** — operator decision gates (Workspace Standing Rule 8: *ADR ratification … do not roll a bespoke gate*). Adopted because three ADRs sat `proposed` with **no decision surface**, so every ruling happened in chat and left no artifact. ⚠ **Pinned by commit, not version: no semver exists for this substrate** — `Astro.aDNA/MANIFEST.md` has no `version:` and `generator.py` no VERSION constant (both checked at the object). Inventing one would be a number the wrapper made up. ⛔ **No `substrate_pin:` field** — the fleet exemplar (`CakeHealth.aDNA`) carries one *beside* `federation_ref`; Canvas removed exactly that duplication at Plumbline P3 (*wrappers point, they never restate*). ⚠ **`mondrian` is not an available persona** (`franklin·hermes·neutral·partner·rosetta·tokyo`) → `neutral`, chosen rather than borrowing another vault's voice. Canvas was **not** among the 10 wrappers in the 2026-07-02 census; it is an 11th. |
