...
title: "Erratum E2 on the diagrammatic-context pattern — its authority axis mixes two questions, and its conformance floor could not be cleared by any canvas for 13 months. Both found by building it, not by re-reading it."
...
created: 2026-09-04
updated: 2026-09-04
status: staged_for_delivery          # ⛔ NOT delivered — refused at the door, see below
delivery_attempted: 2026-09-04
delivery_outcome: "REFUSED (quiet-lease rule). aDNA.aDNA held a LIVE lease at attempt time — `how/sessions/active/session_stanley_20260904_155111_haussmann_deploy_gr5.md`, a parallel operator session opened today — and publishes NO inbound drop-box. Writing into a live-leased peer without a box is exactly what the rule forbids, so nothing was written."
delivery_retry: "Deliver when their lease clears. ⛩ Note the symmetry worth recording: this is the same convention gap Hopper reported against US (their memo reached Canvas only because our lease happened to be clear, and Ilmarinen had four consecutive deliveries into Canvas refused). Canvas opened its own drop-box this session; aDNA.aDNA has not, and today that cost a delivery in the other direction."
direction: outbound
ack_required: false
needs_human: false
...
## E2.2 — ⭐ the conformance floor could not be cleared by any canvas, and had not been for 13 months

The draft's §Conformance floor requires passing the visual gate. Measured this session: **no canvas
containing a titled group could pass `canvas-visual-check --strict`.** Two shipped traps in the same
...
**~14% shown in Obsidian**; `deck_generator`'s example carries 19 findings including 4 `heading_lead`.
The gate is declared mandatory in our own producer skill and was evidently not being run — the trap
corpus grew to 14 while the shipped examples were never re-gated against it.
...
canvases now pass `--strict` clean, including the example that had been failing for 13 months.
