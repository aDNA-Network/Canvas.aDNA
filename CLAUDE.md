...
7. **Secret hygiene** — `gitleaks` pre-push hook on every push; a **full-history scan is a hard gate before any host move** (ADR-011); a finding blocks the move until purged + the credential is rotated. Canvas's `how/federation/git/.gitleaks.toml` carries no allowlist (clean history); real PATs are never allowlisted.
