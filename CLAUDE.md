...
> **Current state:** execution campaign **Operation Keystone** (`how/campaigns/campaign_canvas_genesis/`; the genesis-planning **Operation Cartography** closed 2026-06-13). Built: E0–E2 (the `canvas_std` reference impl + conformance harness + v2.0.0 schema/CLI, 46/8) · E3 (CanvasForge migration + cutover) · **E4 COMPLETE** (4 in-vault consumers on `canvas_std`: `brief_consumer` 10 · `deck_generator` 16 · `document_generator` 37 — E4.1 long-form + E4.2 LF format/visual contracts + section-level reflow) · E5.1 (`iii/` quality wrapper live @ III v0.5.0). **⛔ HELD at the E5→E6 human gate.** Remaining: E5.2 (federation rollout, PT-P5-coupled) · E5.3 (optional Δ2 LIP) · E6 (validation & cutover). Ratified ADRs: `adr_000` (identity) · `adr_004` (production code layout) · `adr_005` (LF-successor in-vault). Detail: `STATE.md`. Phase gates are human gates — never auto-advance.
