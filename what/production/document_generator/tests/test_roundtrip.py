"""Round-trip: topology (incl. the page sequence) is stable across to_canvas/from_canvas."""
...
def test_sequence_chain_is_linear(doc, n_pages):
    pl = doc["metadata"]["frontmatter"]["_reserved"]["panel_link"]
    seq = [eid for eid, m in pl["edges"].items() if m["kind"] == "sequence"]
    assert len(seq) == n_pages - 1  # a single chain across the pages (acyclicity enforced by validate at A-5)
